...
function ResponseDialog({formId,id,onClose}:{formId:string;id:string;onClose:()=>void}) {
  ...
  return <dialog ref={dialog} className="response-dialog" aria-labelledby="response-heading" onCancel={onClose}>
    ...
    {error?<p role="alert">{error}<button className="secondary" onClick={()=>setReload(v=>v+1)}>Retry</button></p>:!detail?<p role="status">Loading response…</p>:<>
      <p>Submitted {time(detail.created_at)}</p><details className="response-version-details"><summary>Version identifier</summary><p className="version-id">{detail.version_id}</p></details><h3>{detail.snapshot.title}</h3>
...
export function Results({formId}:{formId:string}) {
  ...
  return <main className="creator-page results-page">
    <header className="creator-header"><div className="results-breadcrumb"><a href="/"><FileText size={16}/>Forms</a><ChevronRight size={15}/><span>{info?.title||'Results'}</span></div><nav aria-label="Form navigation"><a href={'/?form='+formId}>Content</a><button className="nav-placeholder" aria-disabled="true">Workflow <small>Coming Soon</small></button><button className="nav-placeholder" aria-disabled="true">Connect <small>Coming Soon</small></button><span aria-current="page">Results</span></nav></header>
    ...
    <section className="results-content">
      <div className="results-heading"><h1>{view==='summary'?'Response summary':'Responses'}</h1><p>{view==='summary'?'A breakdown of answers for each question in the selected version.':'Explore individual responses to your form.'}</p></div>
      {error?<div className="error-banner" role="alert">{error}<button onClick={()=>setReload(v=>v+1)}>Retry</button></div>:!info?<p role="status">Loading results…</p>:<>
        ...
        {!version?<div className="workspace-empty"><h2>No published versions yet</h2><p>Publish this form to start collecting responses.</p></div>:versionError?<div role="alert" className="error-banner">{versionError}<button onClick={()=>setReload(v=>v+1)}>Retry</button></div>:!data?<p role="status">Loading version results…</p>:<>
          <p className="version-caption">Version {current?.number} · {data.snapshot.title}</p>
